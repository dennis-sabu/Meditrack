...
export default function DoctorDashboard() {
  ...
  useEffect(() => {
    if (data?.doctor) {
      if (data?.doctor?.hospitalId === null) {
        setNoHospital(true);
      }
    }
  }, [data]);
        ...
        {noHospital && (
          <div className="p-4 rounded-lg bg-red-50 border border-red-200 text-red-700">
            <p className="font-medium">
              You are not associated with any hospital. Please contact admin to
              get verified and linked to a hospital.
            </p>
            <a href="/doctors/my-hospital" className="">
              <button className="mt-2 px-4 py-2 bg-blue-600 text-white rounded-lg">
                Send Request to a hospital
              </button>
            </a>
          </div>
        )}
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          <Card className="border-0 shadow-lg bg-gradient-to-br from-blue-500 to-blue-600 text-white hover:shadow-xl transition-shadow">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-semibold tracking-wide uppercase opacity-90">
                Total Appointments
              </CardTitle>
              ...
              <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-amber-50 to-amber-100 p-6 border border-amber-200 hover:shadow-md transition-shadow">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm font-semibold text-amber-600 mt-1 uppercase tracking-wide">
                      Pending
                    </p>
                  </div>
              ...
              <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-emerald-50 to-emerald-100 p-6 border border-emerald-200 hover:shadow-md transition-shadow">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm font-semibold text-emerald-600 mt-1 uppercase tracking-wide">
                      Completed
                    </p>
                  </div>
            ...
            <div className="space-y-4">
              ...
              data.upcomingAppointments.length > 0 ? (
                data.upcomingAppointments.map((appointment: any) => (
                  <div
                    key={appointment.id}
                    className="flex items-center justify-between p-4 rounded-lg bg-gradient-to-r from-slate-50 to-white border border-slate-200 hover:border-blue-300 hover:shadow-md transition-all"
                  >
            ...
            <div className="space-y-4">
              ...
              data.recentConsultations.length > 0 ? (
                data.recentConsultations.map((consultation: any) => (
                  <div
                    key={consultation.id}
                    className="p-4 rounded-lg bg-gradient-to-r from-slate-50 to-white border border-slate-200 hover:border-blue-300 hover:shadow-md transition-all"
                  >
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-slate-400 mt-1">
                    Consultation records will appear here
                  </p>
                </div>
              )}
            </div>
